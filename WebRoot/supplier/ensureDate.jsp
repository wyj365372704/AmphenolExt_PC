...
<script type="text/javascript">
	...
	window.open('ensureDateModify.jsp'+ encodeURI(flag),'newwindow','height=400,width=400,top=0,left=0,toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no');
	}
		...
		window.location.href="ensureDate.jsp?page="+parseInt(pageno)+"&fwarehouse="+fwarehouse+"&fordrji="+fordrji+"&finfoji="+finfoji+"&fds40ji="+fds40ji;
...
%>
<tr>
	<td><input type="button" value="确认送货日期" onclick="ensureDate('?PISQJI=<%=rs.getString("PISQJI")%>&ORDRJI=<%=rs.getString("ORDRJI")%>&OLDDATE=<%=(rs.getInt("DKDTJI") +Integer.valueOf(19000000))%>&BKSQJI=<%=rs.getString("BKSQJI")%>');"></td>
...
%>
		  ...
		  <div class="page">共有&nbsp;<%=pageCount %>&nbsp;页&nbsp;&nbsp;<%=itemCount %>&nbsp;条记录，当前为第&nbsp;<%=currentPage %>&nbsp;页  
		    <a href="ensureDate.jsp?page=1&fwarehouse=<%=fwarehouse%>&fordrji=<%=fordrji%>&finfoji=<%=finfoji%>&fds40ji=<%=fds40ji%>" title="" onclick="first();">首页</a> 
		    <a href="ensureDate.jsp?page=<%=currentPage-1>0?currentPage-1:1 %>&fwarehouse=<%=fwarehouse%>&fordrji=<%=fordrji%>&finfoji=<%=finfoji%>&fds40ji=<%=fds40ji%>" title="" onclick="pre();">上一页</a> 
		    <a href="ensureDate.jsp?page=<%=currentPage+1>pageCount?pageCount:currentPage+1 %>&fwarehouse=<%=fwarehouse%>&fordrji=<%=fordrji%>&finfoji=<%=finfoji%>&fds40ji=<%=fds40ji%>" title="" onclick="next();">下一页</a> 
		    <a href="ensureDate.jsp?page=<%=pageCount %>&fwarehouse=<%=fwarehouse%>&fordrji=<%=fordrji%>&finfoji=<%=finfoji%>&fds40ji=<%=fds40ji%>" title="" onclick="last();">尾页</a> 每页<%=pageSpace %>条 到第
