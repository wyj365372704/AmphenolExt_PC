...
window.open('<%=request.getContextPath()%>/getShdServlet?shpno='+ shpno+'&stat='+stat,'newwindow','height=800,width=800,top=0,left=0,toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no');
	...
	function gopage(){
		...
		window.location.href="myFhd.jsp?page="+pageno;
...
<body class="right_body">
	<div class="path">您现在的位置： 首页 &gt; Ampheno SRM &gt; 打印送货单</div>
	<form action="myFhd.jsp" method="post" name="buyIn">
	...
	//----------查询总条数
	String sql = "select count(*) as c from " + envId.trim() + ".ZSHPHDR where VNDNR = '" + userCode + "' and OSTAT = '10'";
	if (!"".equals(shdNum)) {
									sql = sql + " AND SHPNO like '%" + shdNum + "%'";
								}
								if (!"".equals(shdDate)) {
									sql = sql + " AND CRDT = " + shdDate;
								}
...
sql = "select * from (select ZSHPHDR.*,rownumber() over() as rn from " + envId.trim()
						...
						<tr>
							...
							<td><%=rs.getString("LGWNO")%></td>
								...
								: rs.getString("OSTAT").equals("30") ? "部分收货"
					  ...
					  <div class="page">共有&nbsp;<%=pageCount %>&nbsp;页&nbsp;&nbsp;<%=itemCount %>&nbsp;条记录，当前为第&nbsp;<%=currentPage %>&nbsp;页    <a href="myFhd.jsp?page=1" title="" onclick="first();">首页</a> <a href="myFhd.jsp?page=<%=currentPage-1>0?currentPage-1:1 %>" title="" onclick="pre();">上一页</a> <a href="myFhd.jsp?page=<%=currentPage+1>pageCount?pageCount:currentPage+1 %>" title="" onclick="next();">下一页</a> <a href="myFhd.jsp?page=<%=pageCount %>" title="" onclick="last();">尾页</a> 每页<%=pageSpace %>条 到第<input type="text" class="page_input" id="gpageno" value="1" />页 <input type="button" value="" class="go_button" onclick="gopage()"/>
			</div>
	</form>
	</div>
